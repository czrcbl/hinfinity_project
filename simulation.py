...
class Simulation:
    ...
    def simulation_step(self, u):
        t = [0, self.Ts]
        if self.nonlinear:
            y1 = odeint(self.func, self.states.flatten(), t, args=(u, self.sys, self.nldat))
        else:
...
#    wheels_vel_vec = np.empty(shape=(N, 3))
    ...
    while True:
        ...
        control_signal[(control_signal > sysdat.actuator_saturation)] = sysdat.actuator_saturation 
